...
from kubernetes.client import RbacAuthorizationV1Api, V1ClusterRoleBinding
from kubernetes.client.rest import ApiException


@resource(
    "kubernetes::resources::ClusterRoleBinding",
    agent="cluster.name",
    id_attribute="identifier",
)
class ClusterRoleBindingResource(ClusterResource):
...
@provider(
    "kubernetes::resources::ClusterRoleBinding", name="cluster-role-binding-resource"
)
class ClusterRoleBindingResourceProvider(CRUDHandler):
    def read_resource(
        self, ctx: HandlerContext, resource: ClusterRoleBindingResource
    ) -> None:
        try:
            api_response: V1ClusterRoleBinding = (
                resource.api_instance.read_cluster_role_binding(resource.name)
            )
    ...
    def create_resource(
        ...
        try:
            api_response = resource.api_instance.create_cluster_role_binding(
    ...
    def delete_resource(
        ...
        try:
            api_response = resource.api_instance.delete_cluster_role_binding(
                resource.name
            )
            ctx.debug(str(api_response))
            while True:
                api_response = resource.api_instance.read_cluster_role_binding(
                    resource.name
                )
    ...
    def update_resource(
        self, ctx: HandlerContext, changes: dict, resource: ClusterRoleBindingResource
    ) -> None:
        try:
            ...
            api_response = resource.api_instance.patch_cluster_role_binding(
